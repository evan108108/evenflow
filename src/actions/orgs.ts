/**
 * Org actions — org CRUD, org members, and board members.
 *
 * EFB-98 split src/routes/orgs.ts in two. Everything that decides what an org
 * IS lives here: the validators, the wire views, the private-board key hooks
 * and the fourteen actions. It imports no Hono and never sees a request.
 * src/routes/orgs.ts is the HTTP shell that extracts params, reads the body,
 ...
 *
 * TWO SLUGS, NAMED APART
 * ----------------------
 * This file is the reason the fan-out brief warned about slugs, so it is worth
 * stating exactly. `params.orgSlug` is the ORG (`:org_slug` in the manifest)
 * and `params.boardSlug` is the BOARD (`:slug`). They are different resources
 * that appear together in the four board-member routes, whose path is
 * `/org/:org_slug/board/:slug/members` — reading one where the other is meant
 * is an authorization bug, not a type error, because both are strings and both
 * resolve to something. The pre-split route never had one name mean two things
 * (org handlers read `org_slug`, board handlers read both), and naming the
 * fields apart here keeps it that way at the call site instead of relying on
 * position.
 *
 * `ActionInput.orgSlug` is deliberately NOT used for this: that field exists
 * for the ten routers mounted twice, bare and under `/org/:org_slug`, where
 * the org is a MOUNT detail. Every route in this family is `orgScoped: false`
 * — its org slug is its own path parameter — so it belongs in `params` with
 * the rest of the path, and duplicating it into the mount field would invite
 * the two copies to disagree.
 */

import { Clock, Effect } from "effect";

import { AuditLog, Audience, Db, DbError, FourA } from "../effects";
import {
  BoardOwnershipError,
  ForbiddenError,
  UnauthorizedError,
...
/** Services every org action needs. */
export type OrgServices = Db | AuditLog | Audience | FourA;

/**
 * Everything an org action can fail with.
 *
 * The shared vocabulary from src/lib/errors.ts rather than local copies:
 * these tags are answered by the shared `errorResponse`, and a second class
 * per tag would be two places for one status code to drift. The classes live
 * in lib/ rather than routes/ so an action can raise one without importing
 * from the route layer.
 */
export type OrgsFailure =
  | ValidationError
  | ConflictError
  | NotFoundError
  | ForbiddenError
...
/** The JSON object a body-reading org route hands its action. */
export type OrgBody = Record<string, unknown>;
...
export const createOrg = (
  input: ActionInput<OrgBody>,
): Effect.Effect<unknown, OrgsFailure, OrgServices> =>
  Effect.gen(function* () {
...
export const deleteOrg = (
  ...
  Effect.gen(function* () {
    const claims = input.claims;
    const { org } = yield* authorizeOrgAccess(input.params["orgSlug"] ?? "", callerPubkey(claims), "owner");
...
export const transferOrg = (
  ...
  Effect.gen(function* () {
    ...
    const pubkey = callerPubkey(claims);
    const body = input.body;
    const { org } = yield* authorizeOrgAccess(input.params["orgSlug"] ?? "", pubkey, "owner");
...
export const listOrgBoards = (
  input: PublicActionInput,
): Effect.Effect<unknown, OrgsFailure, OrgServices> =>
  Effect.gen(function* () {
    const includeArchived = input.query["include_archived"] === "1";
    const pubkey = input.claims === null ? null : callerPubkey(input.claims);
    const resolved = yield* resolveOrgBySlug(input.params["orgSlug"] ?? "");
...
export const listOrgMembers = (
  ...
  Effect.gen(function* () {
    const claims = input.claims;
    const { org } = yield* authorizeOrgAccess(input.params["orgSlug"] ?? "", callerPubkey(claims), "member");
    const db = yield* Db;
    const rows = yield* db.queryAll<Record<string, unknown>>(
...
export const addOrgMember = (
  ...
  Effect.gen(function* () {
    ...
    const pubkey = callerPubkey(claims);
    const body = input.body;
    const { org, role: myRole } = yield* authorizeOrgAccess(input.params["orgSlug"] ?? "", pubkey, "admin");
...
export const addBoardMember = (
  ...
  Effect.gen(function* () {
    ...
    const body = input.body;
    const orgSlug = input.params["orgSlug"];
    const { board } = yield* resolveBoardScope(
