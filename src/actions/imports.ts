...
/**
 * CSV / bulk issue-import actions.
 *
 * EFB-98 split src/routes/imports.ts in two. The route shell resolves params,
 * constructs the body parse and runs runJson; everything that decides what an
 * import DOES lives here.
 *
 * The bodies below moved VERBATIM — the only edit reads the parsed body off
 * `input` instead of calling parseRouteBody against a Context.
 *
 * TWO THINGS WORTH KNOWING BEFORE EDITING:
 *
 *  - The bulk import's parse is DEFERRED (rule 10). It is yielded below the
 *    board gate because that is where it has always run: a malformed body
 *    aimed at a board the caller cannot contribute to answers 404/403, not a
 *    400 about the JSON. The route hands it over un-yielded.
 *
 *  - `createBulkImport` returns `{ replayed, body }` and the ROUTE unwraps
 *    `.body`. `replayed` never reaches the wire and never did — but it is the
 *    domain fact a direct test of this action wants to assert ("this was a
 *    replay, not a fresh import"), so the action keeps it and the formatting
 *    stays in the shell.
 */
...
/** Services the import actions need. */
export type ImportServices = Db | AuditLog | BoardEmitter | Audience;
 ...
 *
 * DEFERRED PARSE (rule 10) — see the file header. `input.body` is the route's
 * un-yielded parseRouteBody; it is yielded below resolveBoardScope, on the
 * line it has always occupied.
 */
export const createBulkImport = (
  input: ActionInput<Effect.Effect<typeof PostBulkIssuesBody.Type, ValidationError, never>>,
): Effect.Effect<{ replayed: boolean; body: unknown }, ImportFailure, ImportServices> =>
  Effect.gen(function* () {
    const claims = input.claims;
    const caller = callerPubkey(claims);
...
export const listImports = (
  input: ActionInput,
): Effect.Effect<unknown, ImportFailure, ImportServices> =>
  Effect.gen(function* () {
    const { board } = yield* resolveBoardScope(
      { org_slug: input.orgSlug ?? undefined, slug: input.params["slug"] ?? "" },
...
export type { PlannedIssue, ImportFailure };
