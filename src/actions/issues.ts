/**
 * Issue actions — issue CRUD, status transitions, and container moves against
 * issueCache, with every state change mirrored into statusChangeCache (the
 * activity-feed source).
 *
 * EFB-98 split src/routes/issues.ts in two, mirroring the comments worked
 * example. Everything that decides what an issue IS lives here; src/routes/
 * issues.ts is the HTTP shell that extracts params, parses the body, calls one
 * of these, and maps a failure to a status code.
 *
 * The bodies below moved VERBATIM. Every comment, every ordering decision and
 * every failure reason is the pre-split code; the only edits read
 * params/body/query/claims off `input` instead of off a Context.
 *
 * WHAT STAYED IN THE ROUTE, and why each one is not laziness:
 *   - `parseRouteBody` and `parseRouteQuery`. check:boundary and
 *     check:boundary-query both scan `src/routes` as TEXT, so a parse moved in
 *     here would make the ratchets blind to it — the exact class of blind spot
 *     EFB-98 exists to close. The schemas live here, with the logic that
 *     consumes them, and the route imports them back.
 *   - `errorResponse`. Failure-union-to-status-code is the one genuinely
 *     transport-shaped part.
 *
 ...
 * through the dedicated endpoint — PATCH deliberately rejects it.
 */
...
// EFB-98 step 6: the failure vocabulary lives in lib/, not routes/. An action
// raises a domain fact without knowing anything about HTTP, so importing these
// from a route file would point the dependency backwards.
import { NotFoundError, QueryValidationError, ValidationError } from "../lib/errors";
import {
  ForbiddenError,
  UnauthorizedError,
  authorizeBoardById,
...
// execute path also needs and which cannot import from a route.
 ...
 * policy or database questions, and they stay in the action as named steps
 * where they already were.
 */
export const ListIssuesQuery = Schema.Struct({
  status: QueryString,
...
/** Services every issue action needs. */
export type IssueServices = Db | AuditLog | BoardEmitter | Audience;
...
// column_id-first mover, and position moves only through the position setter,
// which knows the neighbour midpoint math.
const PATCHABLE = [
  "title",
  "body",
  "body_format",
...
export const PatchIssueBody = Schema.Struct({
 ...
 *
 * Defaults stay in the ACTION, not in the schema. `Schema.optional` with a
 ...
 */
export const PostIssueBody = Schema.Struct({
 ...
 *
 *   `column_id` — stable across renames, and the branch the action takes
 ...
 *     combined in the action with `??`. Nullish coalescing means `to: null`
 *     falls THROUGH to `to_status`, which is why both stay `Unknown` and the
 *     action keeps the `??`: pushing either into the schema would decide the
 ...
 */
export const PostTransitionBody = Schema.Struct({
 ...
 * the resolved issue, so it stays in the action answering `target-is-source`,
 ...
 */
export const PostMoveToBoardBody = Schema.Struct({
  target_board_id: NonEmptyString,
});

/**
 * PUT /issue/:id/position.
 *
 ...
 * present-but-empty. The both-null check stays in the action, where it also
 * covers the explicit-null spelling and keeps answering `neighbors`.
 */
export const PatchReorderBody = Schema.Struct({
 ...
 */
export const ContainerBody = Schema.Struct({
 ...
 * database and a board id the action resolves after parsing: is this person on
 ...
 *
 * DEFERRED PARSE (EFB-98 rule 10). `body` is an un-yielded Effect, not a
 * decoded value, and the odd-looking type is the point: it says out loud that
 * this route's parse ordering is load-bearing and must not be flattened.
 *
 * The pre-split handler resolved the board FIRST and parsed SECOND, so a
 * malformed body sent to a board the caller cannot see answered 404 `board`
 * — not 400. Parsing in the shell before the action runs would flip that to
 * 400 and leak the board's non-existence to anyone willing to send junk.
 * BOUNDARY_DISCIPLINE.md:244: a migration may turn a previously-SILENT failure
 * into a 400, but a 404 that is already a loud, correct answer is on the other
 * side of that line and changing it needs its own ticket.
 */
export const createIssue = (
  input: ActionInput<Effect.Effect<typeof PostIssueBody.Type, ValidationError, never>>,
): Effect.Effect<{ issue: IssueShape }, IssuesFailure, IssueServices> =>
  Effect.gen(function* () {
    ...
    //
    // Yielded HERE, on the line the parse has always sat on: below
    // resolveBoardScope, whose 404/403 must keep winning over this 400.
    ...
    const issue: IssueShape = {
      ...
      // (EFB-30), through the duplicate_of_issue_id field on PATCH.
...
export const listIssues = (
  input: PublicActionInput<typeof ListIssuesQuery.Type>,
): Effect.Effect<unknown, IssuesFailure, IssueServices> =>
  Effect.gen(function* () {
 ...
 *
 * `include` arrives on `input.query` rather than pre-split by the route: it is
 * business input (which expansions the caller wants), and the unknown-key
 * rejection below is this action's answer, not transport's.
 */
...
export const updateIssue = (
  input: ActionInput<typeof PatchIssueBody.Type>,
): Effect.Effect<{ issue: IssueShape }, IssuesFailure, IssueServices> =>
  Effect.gen(function* () {
    const claims = input.claims;
    const pubkey = callerPubkey(claims);
    ...
    yield* emitSecureBoardEvent(
      ...
      moved ? ProvenanceFromCaller(input.claims) : null,
    );
    return { issue };
  });

/** DELETE /issue/:id — cascades comments in code; audit rows stay. */
...
export const transitionIssue = (
  input: ActionInput<typeof PostTransitionBody.Type>,
): Effect.Effect<{ issue: IssueShape }, IssuesFailure, IssueServices> =>
  Effect.gen(function* () {
    const claims = input.claims;
    const pubkey = callerPubkey(claims);
 ...
 */
export const setIssueBoard = (
  input: ActionInput<typeof PostMoveToBoardBody.Type>,
): Effect.Effect<{ issue: IssueShape }, IssuesFailure, IssueServices> =>
  Effect.gen(function* () {
    const claims = input.claims;
    const pubkey = callerPubkey(claims);
    ...
    // about the issue. The route enforces that ordering by parsing first.
    const { target_board_id } = input.body;
 ...
 */
export const setIssuePosition = (
  input: ActionInput<typeof PatchReorderBody.Type>,
): Effect.Effect<{ issue: IssueShape }, IssuesFailure, IssueServices> =>
  Effect.gen(function* () {
    const claims = input.claims;
    const pubkey = callerPubkey(claims);
...
/** POST /issue/:id/container — move between containers, idempotent. */
export const setIssueContainer = (
  input: ActionInput<typeof ContainerBody.Type>,
): Effect.Effect<{ issue: IssueShape }, IssuesFailure, IssueServices> =>
  Effect.gen(function* () {
    const claims = input.claims;
    const pubkey = callerPubkey(claims);
