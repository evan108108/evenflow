/**
 * Invite actions — invite lifecycle: create, resolve (anonymous preview),
 * accept, decline, revoke, email, and per-scope pending lists.
 *
 * EFB-98 split src/routes/invites.ts in two. Everything that decides what an
 * invite IS lives here: the code generator, the state machine, the failure
 * reasons, the persistence, the membership grants. src/routes/invites.ts is
 * the HTTP shell that extracts params, reads the body, calls one of these, and
 * hands the result to runJson.
 *
 ...
 * params/body/claims/token off an `input` record instead of off a Context.
 *
 ...
 *
 * `getInvite` is deliberately anonymous, and its `PublicActionInput` says so
 * in the signature: the preview page renders before sign-in (per spec). It
 * discloses inviter, target, and role for a code the caller already possesses
 * — the code IS the capability.
 *
 * ORG_SLUG. Unlike boards, this router is mounted ONCE, and the two pending-
 * list routes declare `/org/:org_slug/...` in the manifest path itself. It is
 * still business input — it selects which org's invites are being listed — so
 * it still arrives as `input.orgSlug` rather than being read from a Context.
 */

import { Clock, Effect } from "effect";

import { AuditLog, Db, DbError, Email, EmailError, FourA, Audience } from "../effects";
import {
  ForbiddenError,
  UnauthorizedError,
  authorizeOrgAccess,
...
/**
 * Services every invite action needs.
 *
 * FourA rides along because `upsertMembership` publishes the kind-30521 grant
 * that acceptance actually consists of; Audience because a private board mints
 * the accepter an epoch key grant on join.
 */
export type InviteServices = Db | AuditLog | Email | FourA | Audience;
...
/**
 * Load an invite + prove the caller administers its scope.
 *
 * EFB-98: this was a closure inside `makeInvitesRouter`, which meant it was
 * re-created per router instantiation and could not be reached by a test that
...
/**
 * Pending invites for one scope.
 *
 * Same story as `loadInviteForAdmin` — a router-scoped closure that closed
 * over nothing, hoisted to module scope.
 */
 ...
 * POST /invites — create.
 *
 * The body arrives as the raw `Record<string, unknown>` the route read with
 * `readJsonBody`. It is NOT a Schema struct: POST /invites is on
 * check:boundary's `unmigrated` allowlist (sunset 2026-12-31), and the EFB-87
 * re-audit fails an allowlisted route whose file shows no body-read marker —
 * so the `readJsonBody(c)` call stays physically in the route file and this
 * action keeps validating by hand, exactly as it did before the split.
 */
export const createInvite = (
  input: ActionInput<Record<string, unknown>>,
): Effect.Effect<unknown, InvitesFailure, InviteServices> =>
  Effect.gen(function* () {
 ...
 *
 * `PublicActionInput` because this never reads a caller at all: the preview
 * page renders before sign-in, and the code is the capability.
 */
...
/** GET /org/:org_slug/invites — pending invites into the org itself. */
...
/** GET /org/:org_slug/board/:slug/invites — pending invites into one board. */
export const listOrgBoardInvites = (
  input: ActionInput,
): Effect.Effect<unknown, InvitesFailure, InviteServices> =>
  Effect.gen(function* () {
    const claims = input.claims;
